...
# دعم المكتبات الخارجية للطباعة العربية والـ PDF
try:
    from fpdf import FPDF
    import arabic_reshaper
    from bidi.algorithm import get_display # إضافة لدعم اتجاه النص من اليمين لليسار
    ARABIC_SUPPORT = True
except ImportError:
...
class LawyerProApp:
    def __init__(self, root):
        ...
        # تعريف القوائم (الأدوار والمآلات)
    ...
    # --- معالجة اللغة العربية للـ PDF ---
    def ar(self, text):
        if not text or not ARABIC_SUPPORT: return text
        # إعادة تشكيل الحروف العربية وتصحيح الاتجاه (RTL)
    ...
    def open_history_window(self, target):
        ...
        tk.Label(win, text="للتعديل: اختر الدفعة، اكتب المبلغ الجديد، ثم اضغط حفظ", font=(self.main_font_name, 11), bg="#f4f7f6", fg="#7f8c8d").pack(pady=5)
    ...
    # --- الدوال المساعدة للتعديل والبحث والحفظ ---
    ...
    def print_client_statement(self, target):
        ...
        try:
            ...
            pdf.add_page()
            # مسار الخط الافتراضي في ويندوز
            pdf.add_font('TimesAr', '', "C:/Windows/Fonts/times.ttf", uni=True)
            pdf.set_font('TimesAr', '', 18)
            
            # رأس الصفحة
            pdf.cell(0, 10, self.ar(f"كشف حساب الموكل: {target.get('moakal')}"), ln=True, align='C')
            pdf.set_font('TimesAr', '', 14)
            pdf.cell(0, 10, self.ar(f"رقم الملف: {target.get('num')}/{target.get('year')}"), ln=True, align='R')
            pdf.ln(5)

            # معلومات ملخصة
            agreed = float(target.get('total_agreed', 0))
            paid = sum(float(p.get('amount', 0)) for p in target.get('payments_list', []))
            remain = agreed - paid
            
            pdf.set_fill_color(240)
            pdf.cell(60, 10, self.ar(f"المتبقي: {remain:,.2f} دج"), 1, 0, 'C', True)
            pdf.cell(60, 10, self.ar(f"المسدد: {paid:,.2f} دج"), 1, 0, 'C', True)
            pdf.cell(70, 10, self.ar(f"الاتفاق الكلي: {agreed:,.2f} دج"), 1, 1, 'C', True)
            pdf.ln(10)

            # جدول الدفعات التفصيلي
            pdf.set_fill_color(200)
            pdf.cell(100, 10, self.ar("التاريخ والوقت"), 1, 0, 'C', True)
            pdf.cell(90, 10, self.ar("المبلغ المسدد"), 1, 1, 'C', True)

            pdf.set_font('TimesAr', '', 12)
            for p in target.get('payments_list', []):
                pdf.cell(100, 10, str(p.get('date')), 1, 0, 'C')
                pdf.cell(90, 10, f"{float(p.get('amount')):,.2f} دج", 1, 1, 'C')

            fn = f"Statement_{target.get('num')}_{datetime.now().strftime('%H%M%S')}.pdf"
            pdf.output(fn)
            os.startfile(fn)
    ...
    def print_search_results(self):
        ...
        if "الأتعاب" in tab_text:
            ...
            for child in self.fin_tree.get_children():
                ...
                res_to_print.append({"remain": row_vals[0], "paid": row_vals[1], "moakal": row_vals[2], "file_num": row_vals[3]})
            
            if not res_to_print: return
            
            try:
                pdf = FPDF(orientation='P'); pdf.add_page()
                pdf.add_font('TimesAr', '', "C:/Windows/Fonts/times.ttf", uni=True); pdf.set_font('TimesAr', '', 16)
                ...
                pdf.output(fn); os.startfile(fn); return
            except Exception as e: messagebox.showerror("خطأ", str(e)); return

        # طباعة الأقسام الأخرى بنفس المنطق العروبي
        q = self.search_var.get().strip().lower()
        res_data = [x for x in self.cases_list if q in str(x).lower()] if q else self.cases_list
        if not res_data: return
        
        try:
            pdf = FPDF(orientation='L'); pdf.add_page()
            pdf.add_font('TimesAr', '', "C:/Windows/Fonts/times.ttf", uni=True); pdf.set_font('TimesAr', '', 12)
            pdf.cell(0, 10, self.ar(f"جدول القضايا - {datetime.now().strftime('%d/%m/%Y')}"), ln=True, align='C'); pdf.ln(5)
            ...
            fn = f"Print_{datetime.now().strftime('%H%M%S')}.pdf"; pdf.output(fn); os.startfile(fn)
